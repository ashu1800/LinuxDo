...
async function evaluateAndReply(topic, apiKey, chatFn) {
  // Step 1: Worth replying? (use JSON mode for reliable parsing)
  ...
  if (!worthResult.worthReplying || (worthResult.score || 0) < 5) {
    return {
      action: 'skip',
      reason: worthResult.reason || `评分 ${worthResult.score}/10，不值得回复`
    };
  }
  ...
  // Step 3: Safety check (use JSON mode for reliable parsing)
  ...
  try {
    ...
    safetyResult = JSON.parse(safetyText);
  } catch (e) {
